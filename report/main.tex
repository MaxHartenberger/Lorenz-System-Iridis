...
\begin{abstract}
This report presents a computational pipeline for discovering periodic
orbits of the Lorenz system at classical parameters ($\sigma=10$, $\rho=28$,
$\beta=8/3$).  The approach proceeds in two stages.  First, candidate orbits
are identified as \emph{near-recurrences} --- close returns of a long
numerically integrated trajectory --- using a streaming distance-matrix
method that scans narrow time windows around six target periods
$T \in \{5,10,20,40,80,160\}$.  Second, each candidate is refined to a true
periodic orbit via L-BFGS minimisation of a multiple-shooting residual,
with the number of shooting segments $N$ and the L-BFGS memory depth $m$
varied over a $7\times7$ factorial grid ($N,m \in \{5,10,20,40,80,160,320\}$).
The resulting $\sim$29,000 independent optimisation cases are deployed as an
embarrassingly parallel workload on the Iridis~X HPC cluster via SLURM job
arrays.  A systematic post-processing pipeline classifies every case and
generates diagnostic plots (convergence curves, iteration heatmaps, and
aggregate bar charts).  We identify the best $(N,m)$ pair for each target
period and find that short-period orbits ($T \le 20$) converge with
$>\!99\%$ success rate using as few as $N=5$ segments, while longer orbits
require proportionally larger $N$ and consistently benefit from the maximal
L-BFGS memory $m=320$.  The complete pipeline --- implemented in Julia using
\texttt{ToySystems.jl}, \texttt{StreamingRecurrenceAnalysis.jl}, and
\texttt{NKSearch.jl} --- is available at
...
questions such as ``which method reaches $\|F\| = 10^{-6}$ in the shortest
...
\newblock Niels Bohr Institute, Copenhagen, 2016.
...
\bibitem{viswanath2007recurrent}
D.~Viswanath.
\newblock Recurrent motions within plane Couette turbulence.
\newblock \emph{Journal of Fluid Mechanics}, 580:339--358, 2007.
