...
# Compares L-BFGS memory (m) and shooting segments (N) for T ≈ 40 orbits.
# ============================================================================ #
using Pkg
...
mkpath(outdir)
logfile = open(joinpath(outdir, "run_log_T40.txt"), "w")
...
# ---------------------------------------------------------------------------- #
# 2.  Find near-recurrence with T ≈ 40
# ---------------------------------------------------------------------------- #
println("=== Searching for near-recurrence with T ≈ 40 ===")

# Helper: advance state by M timesteps
g(u, M=1) = (ϕ(u, (0, M * Δt)); return u)

# Search for recurrences in index range 3800:4200 → T ∈ [38, 42]
...
recs_iter = recurrences(
    streamdistmat(g, copy(u0), (u, v) -> norm(u - v), 3800:4200, 300_000),
    d -> d < 0.5)
...
for rec in recs_iter
    n_found += 1
    diff = abs(rec[3] * Δt - 40.0)    # rec[3] = Δj = discrete shift
    if diff < best_diff
        best_diff = diff
        best_rec = rec
    end
...
if best_rec === nothing
    error("No near-recurrence found in index range 3800:4200. Try a wider range.")
end
...
serialize(joinpath(outdir, "results_T40_$(timestamp).jls"), results)
serialize(joinpath(outdir, "results_T40_latest.jls"), results)
...
fig.suptitle("L-BFGS Convergence — Lorenz System (T ≈ 40)", fontsize=16, fontweight="bold")
...
fig.tight_layout()
fig.savefig(joinpath(plotdir, "convergence_per_N_T40.pdf"), dpi=150, bbox_inches="tight")
fig.savefig(joinpath(plotdir, "convergence_per_N_T40.png"), dpi=150, bbox_inches="tight")
println("  Saved: convergence_per_N_T40.pdf / .png")

# --- Plot 2: Per-m subplots, each with all N curves ---
fig2, axes2 = plt.subplots(length(Ms), 1, figsize=(12, 4 * length(Ms)), sharex=true)
fig2.suptitle("L-BFGS Convergence — Lorenz System (T ≈ 40)", fontsize=16, fontweight="bold")
...
fig2.tight_layout()
fig2.savefig(joinpath(plotdir, "convergence_per_m_T40.pdf"), dpi=150, bbox_inches="tight")
fig2.savefig(joinpath(plotdir, "convergence_per_m_T40.png"), dpi=150, bbox_inches="tight")
println("  Saved: convergence_per_m_T40.pdf / .png")

# --- Plot 3: Grid of all (N, m) combinations ---
fig3, axes3 = plt.subplots(length(Ns), length(Ms),
                            figsize=(3 * length(Ms), 2.5 * length(Ns)),
                            sharex=true, sharey=true)
fig3.suptitle("L-BFGS Convergence Grid — Lorenz System (T ≈ 40)",
...
fig3.tight_layout()
fig3.savefig(joinpath(plotdir, "convergence_grid_T40.pdf"), dpi=150, bbox_inches="tight")
fig3.savefig(joinpath(plotdir, "convergence_grid_T40.png"), dpi=150, bbox_inches="tight")
println("  Saved: convergence_grid_T40.pdf / .png")
...
ax4.set_title("Final Residual vs Memory — Lorenz System (T ≈ 40)")
ax4.legend()
ax4.grid(true, alpha=0.3)
ax4.set_xticks(Ms)
fig4.tight_layout()
fig4.savefig(joinpath(plotdir, "final_residual_vs_m_T40.pdf"), dpi=150, bbox_inches="tight")
fig4.savefig(joinpath(plotdir, "final_residual_vs_m_T40.png"), dpi=150, bbox_inches="tight")
println("  Saved: final_residual_vs_m_T40.pdf / .png")
...
ax5.set_title("Converged Period vs Memory — Lorenz System (T ≈ 40, converged only)")
ax5.legend()
ax5.grid(true, alpha=0.3)
ax5.set_xticks(Ms)
fig5.tight_layout()
fig5.savefig(joinpath(plotdir, "final_period_vs_m_T40.pdf"), dpi=150, bbox_inches="tight")
fig5.savefig(joinpath(plotdir, "final_period_vs_m_T40.png"), dpi=150, bbox_inches="tight")
println("  Saved: final_period_vs_m_T40.pdf / .png")
...
println("\nDone.")
    ...
    close(logfile)
    println("Log saved to $(joinpath(outdir, "run_log_T40.txt"))")
end
end # main()
