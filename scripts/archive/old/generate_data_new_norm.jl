...
function run_one_case(N::Int, m::Int,
    ...
    opts = Options(
        ...
        callback         = cb,
        e_norm_type     = :max_segment,     # use max-segment norm for convergence (‖F‖_max_seg),
    )
    ...
    final_normF = maximum(norm, b_tmp.x)   # max-segment norm, consistent with e_norm_type=:max_segment
...
function main()
    # --- Parameters -----------------------------------------------------------
    T_targets = [5.0, 10.0, 20.0, 40.0, 80.0]   # target orbit periods
    Ms        = [1, 2, 5, 10, 20, 40]            # L-BFGS memory sizes
    Ns        = [1, 2, 5, 10, 20]                # number of shooting segments
